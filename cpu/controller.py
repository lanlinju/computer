...
def compile_addr2(addr, ir, psw, index):
    ...
    # 1xxx [dd][ss]
    op = ir & 0xf0   # 1xxx_0000, 例如 MOV: 1000_0000
    amd = (ir >> 2) & 3  # 取 3-2 位，目标操作数 (Address Mode Destination)
    ams = ir & 3     # 取 1-0 位，源操作数 (Address Mode Source)

    INST = ASM.INSTRUCTIONS[2]  # 取二地址指令表
    if op not in INST:
        micro[addr] = pin.CYC
        return
    am = (amd, ams)
...
print('Compiling micro instructions finished')
